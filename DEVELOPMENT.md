...
test/                  Node で動く自動テスト（AE 不要・依存パッケージ不要）
  ...
  stage-model.test.js  立ち絵タブのクリック時保証（制御ヌル自動作成・既定マーカー）
  psd.test.js          セットアップ（走査・名前正規化・リネーム移行・使用中更新）
  stage-model.test.js  立ち絵タブのクリック時保証（制御ヌル自動作成・既定マーカー）
...
- **セットアップ＝名前の正規化（全グループ・選択なし）＋使用中グループの式更新**。
  式の登録・制御レイヤー作成は立ち絵タブのクリック時（`ensureCtrlLayerForNode`）。
  制御レイヤーが無いグループはパス（未使用＝式ゼロで軽い）。名前は常に全件を
  一括で正規化し、「一部だけリネーム済み」という状態を作らない（リネーム時は
  `migrateGroupRename` がマーカー・表情セット・式内参照を新名へ移行する。
  AE はコンポリネームで参照フォルダレイヤー名が追従変更されるため必須）。
- **立ち絵タブのルート候補 = セットアップ済み（comment の `emoSetup` タグ）or
  制御レイヤーあり**。セットアップのルート選択は自由（PSD 以外の手組みも可）。
- **制御コンポの指定はルートコンポの comment に `emoCtrl=<名前>`（行形式）で記録**。
  立ち絵タブが制御レイヤーを作るときは「既存式の参照 → emoCtrl タグ → ルート自身」
  の順で解決する（コンポ名は空白を含み得るため空白区切りタグにしない）。
- **未登録（式なし）レイヤーの表示は enabled が真実**。マーカー運用に合流する
  瞬間（クリック時の登録）に見た目が変わらないよう、表示中だった未登録レイヤーは
  既存の全マーカー集合へ補われる（`mergeVisibleIntoMarkers`。排他は未代表の
  集合のみ）。タブのチェック表示も `augmentVisibleSetWithStatic` で合成する。
